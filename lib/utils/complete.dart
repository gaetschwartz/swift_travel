  ...
  String query,
    ...
    Future.microtask(() {
      ...
      for (final c in store.favorites) {
        ...
        final double leven =
            levenshtein(query, c.completion.label.replaceAll(",", "")) / c.completion.label.length;
        // Skip if above threshold
        if (leven < _kConfidenceThreshold) {
          // log("$query - ${c.label} => $leven");
          levens[c.completion] = leven;
        }
      }

      final List<MapEntry<CffCompletion, double>> favs = levens.entries.toList();
